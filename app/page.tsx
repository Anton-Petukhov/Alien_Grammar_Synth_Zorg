...
const PARAMETER_LABELS: Array<{
  ...
  { key: "coherence", label: "Связность", hint: "насколько жесты складываются в узнаваемый контур" },
  ...
  { key: "irreversible", label: "Необратимость", hint: "внутренние связи меняются навсегда" },
  { key: "loss", label: "Ошибка перевода", hint: "часть смысла теряется при превращении в звук" },
...
function formatParameter(key: keyof Parameters, value: number) {
  if (key === "polyphony") return String(1 + Math.floor(value * 31));
  if (key === "speed") {
    const multiplier = 0.25 * Math.pow(16, value);
...
export default function Home() {
  ...
  const ensureEngine = useCallback(async () => {
    ...
    await context.audioWorklet.addModule("/alien-processor.js?v=13");
  ...
  const toggleRunning = useCallback(async () => {
    try {
      ...
      setMessage("Браузер не разрешил запустить звук");
    }
        ...
        <header className="flex flex-col gap-4 border-b border-[var(--line)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7">
          <div>
            <div className="flex items-center gap-3">
              ...
              <p className="eyebrow">EXPERIMENT 06 / CULTURAL EXTINCTION</p>
            </div>
          ...
          <p className="max-w-xl text-sm leading-6 text-[var(--muted-ink)] sm:text-right">
            Три субкультуры спорят, скрещиваются и переживают смену вкусов, новые органы и великие музыкальные вымирания.
          </p>
        </header>
        ...
        <section className="grid flex-1 lg:grid-cols-[minmax(0,1fr)_370px]">
          <div className="flex min-h-[480px] flex-col border-b border-[var(--line)] lg:border-r lg:border-b-0">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] px-5 py-3 text-xs text-[var(--muted-ink)] sm:px-7">
              ...
              <div className="flex flex-wrap justify-end gap-x-5 gap-y-1 font-mono text-[11px] uppercase tracking-[0.08em]">
                <span>семя {formatSeed(graph.seed)}</span>
                <span>возраст {graph.evolutionYear}</span>
                <span>гений {Math.round(graph.evolutionScore)}</span>
            ...
            <div className="relative flex-1 p-3 sm:p-5">
              <canvas
                ...
                aria-label="Поле контакта с причинной сетью"
                role="application"
                tabIndex={0}
                onPointerDown={(event) => {
              ...
              {graph.evolving && (
                <div className="pointer-events-none absolute inset-3 flex items-center justify-center bg-[rgba(8,10,9,0.72)] sm:inset-5">
                  <div className="w-[min(420px,80%)] text-center">
                    ...
                    <p className="mt-4 text-[11px] leading-5 text-[var(--muted-ink)]">
                    ...
                    <p className="mt-2 font-mono text-[10px] uppercase tracking-[0.08em] text-[#a890ff]">
                      18 музыкантов · Ω-1/2/3: {graph.subcultureSizes.join("/")} · убежища: {graph.protectedGeniuses}
                    </p>
                  </div>
                </div>
              )}
              <div className="pointer-events-none absolute right-7 bottom-7 text-[10px] uppercase tracking-[0.14em] text-[var(--muted-ink)]">
                родство → · напряжение ↑
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 border-t border-[var(--line)] bg-[var(--panel-2)]">
              <div className="px-3 py-3 text-center sm:px-5">
                ...
                <p className="mt-1 hidden text-[10px] text-[var(--muted-ink)] sm:block">связность</p>
              </div>
              <div className="border-l border-[var(--line)] px-3 py-3 text-center sm:px-5">
                ...
                <p className="mt-1 hidden text-[10px] text-[var(--muted-ink)] sm:block">тема · крики восторга</p>
              </div>
              <div className="border-l border-[var(--line)] px-3 py-3 text-center sm:px-5">
                <p className="eyebrow">КРИТИКИ</p>
                <p className="mt-1 font-mono text-lg text-[var(--acid)]">{approval(graph.criticApproval)}</p>
                <p className="mt-1 hidden text-[10px] text-[var(--muted-ink)] sm:block">риск · развитие</p>
              </div>
              <div className="border-l border-[#51426e] bg-[rgba(168,144,255,0.035)] px-2 py-3 text-center sm:px-5">
                ...
                <p className="mt-1 hidden text-[10px] text-[var(--muted-ink)] sm:block">
            ...
            {graph.culturalEnabled && (
              <div className="grid grid-cols-3 border-t border-[#3d3651] bg-[rgba(168,144,255,0.035)] sm:grid-cols-6">
                ...
                <div className="col-span-2 border-l border-t border-[#3d3651] px-2 py-3 text-center sm:col-span-1 sm:border-t-0">
                  ...
                  <p className="mt-1 truncate text-[11px] text-[#b9a8ff]">{graph.latestInvention}</p>
                </div>
              </div>
            )}

            <div className="flex flex-wrap items-center justify-between gap-2 border-t border-[#3d3651] bg-[rgba(168,144,255,0.035)] px-4 py-2 font-mono text-[10px] uppercase tracking-[0.08em] text-[#b9a8ff] sm:px-5">
            ...
            <div className="grid gap-3 border-t border-[var(--line)] p-4 sm:grid-cols-3 sm:p-5 xl:grid-cols-6">
              <Button size="lg" onClick={toggleRunning}
                className="h-12 rounded-none bg-[var(--acid)] text-[#07110d] hover:bg-[#a5ffd8]">
                {graph.evolving
                  ? running ? "Остановить эволюцию" : "Продолжить эволюцию"
                  : running ? "Остановить" : ready ? "Продолжить" : "Запустить"}
              </Button>
              ...
              <Button size="lg" variant="outline" onClick={culturalRevolution}
                disabled={!graph.hasComposer || graph.evolving || graph.culturalEnabled}
                ...
                {graph.culturalEnabled ? "Революция активна" : "Культурная революция"}
              </Button>
              ...
              {recordingUrl && recordingFile && (
                <Button size="lg" variant="outline" onClick={saveRecording}
                  className="h-12 rounded-none border-[var(--acid)] bg-transparent text-[var(--acid)] hover:bg-[rgba(125,255,203,0.08)] hover:text-[var(--acid)] sm:col-span-3 xl:col-span-6">
          ...
          <aside className="flex flex-col bg-[var(--panel-2)]">
            <div className="border-b border-[var(--line)] px-5 py-5 sm:px-6">
              ...
              <p className="mt-2 text-sm leading-6 text-[var(--muted-ink)]">
                Восемнадцать музыкантов разделены на три субкультуры. Они скрещиваются, изобретают звуковые органы и получают временное убежище после гениальной мутации. Климат Ω-{cultureCode} больше не гарантирует спокойного будущего.
              </p>
            </div>
                ...
                <div key={parameter.key} className="px-5 py-4 sm:px-6">
                  <div className="mb-3 flex items-baseline justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-[11px] leading-4 text-[var(--muted-ink)]">{parameter.hint}</p>
                    </div>
                  ...
                  />
                  {parameter.key === "mood" && (
                    <div className="mt-2 flex justify-between font-mono text-[9px] uppercase tracking-[0.08em] text-[var(--muted-ink)]">
                      <span>веселее</span>
            ...
            <div className="border-t border-[var(--line)] px-5 py-4 text-[11px] leading-5 text-[var(--muted-ink)] sm:px-6">
              Каждые 500–1700 лет меняются вкусы, а каждые 1700–5300 лет происходит автоматическое вымирание. Кнопка запускает катастрофу немедленно: остаются два музыканта, после чего возникает новая популяция и один неизвестный прежде звуковой орган.
            </div>
          </aside>
        </section>
      </div>
