class AlienGrammarProcessor extends AudioWorkletProcessor {
  ...
  createGenome(parentA = null, parentB = null) {
    ...
    for (let i = 0; i < this.genomeLength; i += 1) {
      ...
      modes[i] = this.random() < 0.5 ? parentA.modes[i] : parentB.modes[i];
      durations[i] = this.random() < 0.5
        ? parentA.durations[i]
        : parentB.durations[i];
  ...
  scoreGenome(genome) {
    ...
    if (this.culturalEnabled && this.culturalArchive.length) {
      ...
      totalScore -= stagnation * (9 + Math.min(18, this.cultureEpoch * 0.42));
    }
  ...
  deserializeGenome(data) {
    ...
    return {
      modes: Uint8Array.from(data.modes),
      durations: Uint8Array.from(data.durations),
      energies: Uint8Array.from(data.energies),
  ...
  restoreSnapshot(snapshot) {
    try {
      ...
      this.culturalEnabled = Boolean(snapshot.culturalEnabled);
  ...
  continueEvolution(years, advanceCulture = true) {
    ...
    const champion = this.cloneGenome(this.composerGenome);
    champion.score = this.scoreGenome(champion);
    this.evolutionPopulation.length = 0;
    this.evolutionPopulation.push(champion);
    for (let i = 1; i < 18; i += 1) {
      const outsider = this.createGenome();
      this.evolutionPopulation.push(this.createGenome(champion, outsider));
    }
  ...
  beginCulturalRevolution() {
    if (this.evolving || !this.composerGenome || this.culturalEnabled) return;
  ...
  evolveChunk() {
    for (let i = 0; i < 2 && this.evolving; i += 1) {
  ...
  reportState() {
    ...
    this.port.postMessage({
      ...
      events: this.eventCount, voices: this.voices.length, seed: this.seed,
  ...
  process(_inputs, outputs) {
    ...
    if (this.awake) {
      if (this.evolving) this.evolveChunk();
      if (!this.evolving || this.composerGenome) {
        this.dramaturgyCountdown -= left.length;
        if (this.dramaturgyCountdown <= 0) {
          this.dramaturgyCountdown = 1024;
          this.updateDramaturgy(false);
        }
      }
    }
    const culturalAnatomy = this.culturalEnabled && this.composerGenome
      ? this.composerGenome.anatomy
      : null;
    for (let sample = 0; sample < left.length; sample += 1) {
      if (this.awake && (!this.evolving || this.composerGenome)) {
        this.samplesUntilEvent -= 1;
        if (this.samplesUntilEvent <= 0) this.createEvent();
        this.sampleClock += 1;
      }
      let valueL = 0;
      let valueR = 0;
      for (let index = this.voices.length - 1; index >= 0; index -= 1) {
        ...
        const value = this.renderVoiceSample(voice) * voice.gain;
        valueL += value * voice.gainL;
        valueR += value * voice.gainR;
        if (voice.position >= voice.length) this.voices.splice(index, 1);
      }
      const master = 0.88 / (1 + Math.max(0, this.voices.length - 4) * 0.08);
      const directL = valueL * master;
      const directR = valueR * master;
      const excitation = (directL + directR) * 0.5;
      let bodyL = 0;
      let bodyR = 0;
      for (let index = 0; index < this.resonators.length; index += 1) {
        const resonator = this.resonators[index];
        const raw = excitation * resonator.inputGain
          + resonator.coefficient * resonator.y1
          - resonator.radiusSquared * resonator.y2;
        const rawSquared = raw * raw;
        const resonated = raw * (27 + rawSquared) / (27 + 9 * rawSquared);
        resonator.y2 = resonator.y1;
        resonator.y1 = Math.abs(resonated) < 1e-18 ? 0 : resonated;
        bodyL += resonated * resonator.outputGain * resonator.gainL;
        bodyR += resonated * resonator.outputGain * resonator.gainR;
      }
      const resonanceAmount = this.params.drama
        * (0.32 + this.currentTension * 0.68)
        * (0.46 + this.params.coherence * 0.54);
      const directAmount = 1 - resonanceAmount * 0.31;
      let mixedL = directL * directAmount + bodyL * resonanceAmount;
      let mixedR = directR * directAmount + bodyR * resonanceAmount;
      if (culturalAnatomy) {
        const percussion = this.renderCulturePercussion(
          culturalAnatomy,
          this.composerGenome.organs,
        );
        mixedL += percussion * 0.96;
        mixedR += percussion * 0.92;
        const brightness = culturalAnatomy[3];
        const drive = culturalAnatomy[0];
        const tone = 0.09 + brightness * 0.35;
        this.masterToneL += (mixedL - this.masterToneL) * tone;
        this.masterToneR += (mixedR - this.masterToneR) * tone;
        const masterDrive = 1 + drive * 4.8;
        const wetL = Math.tanh(this.masterToneL * masterDrive);
        const wetR = Math.tanh(this.masterToneR * masterDrive);
        const wet = drive * 0.48;
        left[sample] = Math.tanh(mixedL) * (1 - wet) + wetL * wet;
        right[sample] = Math.tanh(mixedR) * (1 - wet) + wetR * wet;
      } else {
        left[sample] = Math.tanh(mixedL);
        right[sample] = Math.tanh(mixedR);
      }
      if (this.recording) {
